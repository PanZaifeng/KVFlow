...
class LoRAHiRadixCache(LoRARadixCache):
    ...
    def __init__(
        ...
        enable_interrupt: Optional[bool] = False,
        enable_holding: Optional[bool] = False,
    ):
        ...
        self.enable_holding = enable_holding
    ...
    def write_backup(self, node: LoRATreeNode, write_back=False):
        logger.debug(f"Write back node {node.id} to host, len {len(node.key)}")
            ...
            try:
                ...
                if node_id in self.ongoing_load_back:
                    ...
                    if success:
                        ...
                        logger.debug(f"[Load][Result] Node {end_node.id} loading successfully")
                    else:
                        self.dec_lock_ref(end_node)
                        while cur_node != start_node:
                            assert cur_node.loading
                            ...
                            cur_node = cur_node.parent
                        logger.error(f"[Load][Result] Node {end_node.id} loading failed")

                    del self.ongoing_load_back[node_id]
                else:
                    logger.error(f"[Check][Load] Node {node_id} loading not in progress")
    ...
    def _evict_helper(self, num_tokens: int, ignore_holding: bool = False, steps: int = 1):
        ...
        if len(leaves) == 0:
            logger.info(f"\033[33m [Evict][holding={not ignore_holding}] No more leaves to evict \033[0m")
        logger.debug(f"\033[33m [Evict] evict len = {num_evicted} \033[0m")
    ...
    def load_back(
        ...
        if len(nodes_to_load) == 0:
            logger.debug(f"\033[94m [load][return]    no nodes to load back, node-id:{node.id}, node-evicted:{node.evicted}, node-loading:{node.loading} \033[0m")
            return None
        total_len = sum([len(n.key) for n in nodes_to_load])
        logger.debug(f"\033[94m [Load][init] back priority={priority} total_len={total_len} \033[0m")
        ...
        logger.debug(f"\033[94m [Load][submit] back priority={priority} total_len={total_len} \033[0m")
    ...
    def get_node_chain_status(self, req_last_node: LoRATreeNode):
        ...
        while n != self.root_node:
            if n.evicted:
                logger.debug(f"[status]   evicted id: {n.id}")
                return self.REQ_IS_EVICTED
            if n.loading:
                logger.debug(f"[status]   loading id: {n.id}")
                return self.REQ_IS_LOADING
            n = n.parent
            logger.debug(f"[status] node id: {n.id}, evicted: {n.evicted}, loading: {n.loading}")
    ...
    def _update_leaf_node_timestep(self):
        ...
        logger.debug("[Hold][Update] Leaf node priorities updated.")
        return
