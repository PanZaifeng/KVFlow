...
class Scheduler(
    ...
    def __init__(
        ...
        self._request_dispatcher = TypeBasedDispatcher(
            [
                ...
                (UpdateAgentTimestepReq, self.HandleReq),
            ...
            if os.environ.get("SGLANG_EXPERIMENTAL_CPP_RADIX_TREE") == "1":
                ...
                if self.enable_lora:
                    ...
                    self.tree_cache = LoRAHiRadixCache(
                        ...
                        enable_interrupt=self.server_args.enable_interrupt,
                        enable_holding=self.server_args.enable_holding
                    )
                else:
            ...
            if batch:
                if self.is_idle == True:
                    ...
                    if self.new_timestep == True:
                        ...
                        self.timestep_start = time.perf_counter()
                        logger.info("\033[95m   [BD] New Timestep, Start to Count   \033[0m")
    ...
    def get_new_batch_prefill(self) -> Optional[ScheduleBatch]:
        ...
        for req in waiting_queue_iter:
            ...
            if self.tree_cache is not None and self.enable_hierarchical_cache:
                ...
                logger.debug(f"[aid: {req.agent_id}] Request {req.rid} Node {req.last_host_node.id} evicted {req.last_host_node.evicted} loading {req.last_host_node.loading} loading status: {loading_status}")
    ...
    def run_batch(
        ...
        if self.is_generation:
            ...
            #     logger.debug(f"[Prefill] {current_extend_token}")
        ...
        if hasattr(self, 'agent_manager') and self.agent_manager is not None:
            try:
                if recv_req is None:
                    ...
                    return
                logger.info(f"Received agent timestep update request: {recv_req.agent_data}, {recv_req.timestep_data}, {recv_req.timestep_cnt}")
                ...
                last_agent_pool_list = sorted(self.last_agent_pool)
                missing_activate_agent_ids = [
                    agent_id
                    for agent_id in activate_agent_list
                    if f"lora{agent_id}" not in last_agent_pool_list
                ]
                ts = recv_req.timestep_cnt
                logger.info(f"\033[94m [{ts} Ts][Statistic] \033[0m: [Activate Agent ({len(activate_agent_list)})] {activate_agent_list}")
                logger.info(f"\033[94m [{ts} Ts][Statistic] \033[0m: [Initial Agent Pool] {last_agent_pool_list}")
                logger.info(f"\033[94m [{ts} Ts][Statistic] \033[0m: [Prefetch Agent Pool] {self.lora_manager.memory_pool.list_loaded_lora_names()}")
                if missing_activate_agent_ids:
                    logger.info(f"\033[94m [{ts} Ts][Statistic] \033[0m: [Cache-Miss Agent] {len(missing_activate_agent_ids)} {missing_activate_agent_ids}")
                if not self.server_args.disable_prefetch:
                    self.DispatchLoadTasks(self.server_args.load_ahead_step)
                end_time = time.time()                
                lasting_time = end_time - self.timestep_start
                rate = self.decode_token_count / self.prefill_token_count if self.prefill_token_count > 0 else -1
                logger.info(f"\033[94m [{ts} Ts][Statistic] \033[0m:  [Prefill Token: {self.prefill_token_count}][Decode Token: {self.decode_token_count}][rate: {rate:.4f}]")
                logger.info(f"\033[94m [{ts} Ts][Statistic] \033[0m:  [{lasting_time:.4f}s][{self.req_nums} req][{self.batch_per_timestep} bs] Updated timestep data: {recv_req.timestep_data}, Updated agent data: {recv_req.agent_data} evict: {self.tree_cache.evictable_size()}")
                self.last_agent_pool = self.lora_manager.memory_pool.list_loaded_lora_names()
                self.batch_per_timestep = 0
                self.activate_agent = set()
                self.new_timestep = True
                self.req_nums = 0
