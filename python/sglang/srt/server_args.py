...
class ServerArgs:
    ...
    def add_cli_args(parser: argparse.ArgumentParser):
        ...
        parser.add_argument(
            ...
            type=str,
            default=None,
            help="Path to model weights (local folder or HF repo). Optional if provided via --config.",
        )
        parser.add_argument(
            "--tokenizer-path",
            type=str,
        ...
        parser.add_argument(
            "--enable-holding",
            action="store_true",
            help="enable priority-based eviction/holding"
        )
        parser.add_argument(
            "--enable-interrupt",
            action="store_true",
            help="Enable interrupt in loading",
        )
        parser.add_argument(
            "--self-debug",
            action="store_true",
...
def prepare_server_args(argv: List[str]) -> ServerArgs:
    """
    Prepare server args from CLI and an optional JSON/YAML config.
    Merge order: config defaults -> CLI overrides. model_path must exist in one of them.
    """

    # Parse --config early so the full parser can inherit it for help text.
    config_only_parser = argparse.ArgumentParser(add_help=False)
    config_only_parser.add_argument(
        "--config",
        type=str,
        help="Path to a JSON or YAML file containing ServerArgs fields.",
    )
    config_args, remaining_argv = config_only_parser.parse_known_args(argv)
    ...
    def _load_config(path: str) -> dict:
        ...
        valid_keys = {field.name for field in dataclasses.fields(ServerArgs)}
        filtered = {k: v for k, v in loaded.items() if k in valid_keys}
        unknown = set(loaded.keys()) - valid_keys
        if unknown:
    ...
    # Build the real parser (inherits --config so it shows in --help)
    parser = argparse.ArgumentParser(parents=[config_only_parser])
    ServerArgs.add_cli_args(parser)
    if config_defaults:
        parser.set_defaults(**config_defaults)

    raw_args = parser.parse_args(remaining_argv)
    # Drop helper flag before constructing the dataclass
    if hasattr(raw_args, "config"):
        delattr(raw_args, "config")

    server_args = ServerArgs.from_cli_args(raw_args)
    if server_args.model_path is None:
        raise ValueError("model_path must be provided via --model-path or --config.")
