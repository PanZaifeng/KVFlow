...
class LoRAManager:
    ...
    def prefetch_lora_weights(self, priority: int = 0, lora_id: str = None, step_lora_ids: List[str] = []) -> bool:
        ...
        # logger.debug(f"\033[94m [lora][Prefetch] \033[0m    Try to prefetch LoRA adapter weights: {lora_name} with priority {priority}, step lora ids is in info level")
        # logger.debug(f"\033[94m [lora][Prefetch] \033[0m    step_lora_ids: {step_lora_ids}")
    ...
    def prepare_lora_batch(self, forward_batch: ForwardBatch):
        ...
        # logger.debug(f"\033[94m [lora][Prepare]\033[0m Preparing LoRA batch with LoRA IDs: {cur_names}")
    ...
    def get_and_update_lora_time(self):
        delta_prepare = self.prepare_lora_time
        self.prepare_lora_time = 0
        return delta_prepare